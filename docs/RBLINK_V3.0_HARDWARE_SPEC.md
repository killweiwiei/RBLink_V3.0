...
- 复位阶段由100 kΩ下拉保持四路关闭；当前固件取得GPIO控制权后默认将JTAG/SWD、UART、
  SPI和I2C四路PORTEN全部置高。用户关闭某接口时再将对应PORTEN置低，使目标侧进入高阻。
...
4. **PORTEN默认状态已确认。** R59/R60/R63/R67均为100kΩ下拉，MCU复位期间接口关闭；
   应用固件初始化完成后默认打开全部四路电平转换器。
